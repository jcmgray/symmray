...
def test_flat_decomposition_backend_roundtrip(
    method,
    backend,
    require_backend,
):
    ...
    x = (
        get_zn_blocksparse_flat_compat(
            "Z2",
            shape=[8, 6],
            seed=42,
        )
...
def test_flat_svd_truncated_backend(method, backend, require_backend):
    require_backend(backend)
    x = (
        get_zn_blocksparse_flat_compat(
            "Z2",
...
def test_flat_eigh_backend(truncated, backend, require_backend):
    require_backend(backend)
    x = sr.utils_test.rand_matrix(
        "Z2",
        4,
...
def test_flat_cholesky_backend(regularized, backend, require_backend):
    require_backend(backend)
    x = (
