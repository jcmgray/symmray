...
- In MyST markdown, use a link with a leading `#`:
  `` [`AbelianArray`](#AbelianArray) ``, `` [`fuse`](#AbelianArray.fuse) ``.
  This is the preferred form, since it also renders cleanly as plain markdown.
- In a docutils role, use a leading `.`: `` {class}`.AbelianArray` ``,
  `` {meth}`.AbelianArray.fuse` ``. Roles are typed, so prefer them in
