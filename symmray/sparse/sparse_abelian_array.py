...
class AbelianArray(
    ...
    def svd_via_eig(self):
        """Singular value decomposition of this sparse abelian array,
        using eigen-decomposition of the Gram matrix
        (xdag @ x or x @ xdag). This can be faster, but also incur a
        loss of precision due to the squaring.
    ...
    def svd_via_eig_truncated(
        ...
        **kwargs,
        ...
        abelian array, using eigen-decomposition of the Gram matrix
        (xdag @ x or x @ xdag). This can be faster, but also incur
        a loss of precision due to the squaring.
        ...
        absorb : {-1, 0, 1, None}
            ...
            - None: do not absorb, return singular values as a
              BlockVector.

        renorm : {0, 1}
            Whether to renormalize singular values (depends on
            ``cutoff_mode``).

        Returns
        -------
        u : AbelianArray or None
            The abelian array of left singular vectors.
        s : BlockVector or None
            The vector of singular values, or None if absorbed.
        vh : AbelianArray or None
