from importlib.metadata import PackageNotFoundError as _PackageNotFoundError
from importlib.metadata import version as _version

try:
    __version__ = _version("symmray")
except _PackageNotFoundError:
    try:
        # fallback for source trees where hatch-vcs has generated _version.py.
        from ._version import version as __version__
    except ImportError:
        __version__ = "0.0.0+unknown"

from . import linalg, scipy, utils, utils_test
from .fermionic_local_operators import (
    FermionicOperator,
    build_local_fermionic_array,
    build_local_fermionic_dense,
    build_local_fermionic_elements,
    fermi_double_occupancy_local_array,
    fermi_hubbard_local_array,
    fermi_hubbard_spinless_local_array,
    fermi_number_down_local_array,
    fermi_number_operator_spinful_local_array,
    fermi_number_operator_spinless_local_array,
    fermi_number_up_local_array,
    fermi_pairing_bond_local_array,
    fermi_pairing_onsite_local_array,
    fermi_spin_minus_local_array,
    fermi_spin_operator_local_array,
    fermi_spin_plus_local_array,
    fermi_spin_z_local_array,
    get_spinful_charge_indexmap,
    get_spinless_charge_indexmap,
)
from .flat.flat_abelian_array import (
    AbelianArrayFlat,
    Z2ArrayFlat,
)
from .flat.flat_fermionic_array import (
    FermionicArrayFlat,
    Z2FermionicArrayFlat,
)
from .flat.flat_vector import FlatVector
from .hamiltonians import (
    ham_fermi_hubbard_from_edges,
    ham_fermi_hubbard_spinless_from_edges,
    ham_heisenberg_from_edges,
    ham_tfim_from_edges,
)
from .interface import (
    abs,
    align_axes,
    all,
    any,
    clip,
    conj,
    einsum,
    expand_dims,
    fuse,
    imag,
    isfinite,
    lddiv,
    ldmul,
    log,
    log2,
    log10,
    max,
    min,
    multiply_diagonal,
    rddiv,
    rdmul,
    real,
    reshape,
    sqrt,
    squeeze,
    sum,
    take,
    tensordot,
    trace,
    transpose,
)
from .networks import (
    MPS_abelian_rand,
    MPS_fermionic_rand,
    PEPS3D_abelian_rand,
    PEPS3D_fermionic_rand,
    PEPS_abelian_rand,
    PEPS_fermionic_rand,
    TN_abelian_from_edges_rand,
    TN_fermionic_from_edges_rand,
    parse_edges_to_site_info,
)
from .sparse.sparse_abelian_array import (
    AbelianArray,
    U1Array,
    U1U1Array,
    Z2Array,
    Z2Z2Array,
)
from .sparse.sparse_array_common import (
    BlockIndex,
    default_tensordot_mode,
    get_default_tensordot_mode,
    set_default_tensordot_mode,
)
from .sparse.sparse_fermionic_array import (
    FermionicArray,
    U1FermionicArray,
    U1U1FermionicArray,
    Z2FermionicArray,
    Z2Z2FermionicArray,
)
from .sparse.sparse_vector import (
    BlockVector,
)
from .spin_local_operators import (
    build_local_spin_array,
    build_local_spin_dense,
    get_spinhalf_charge_indexmap,
    heisenberg_local_array,
    spin_operator_local_array,
    tfim_local_array,
)
from .symmetries import (
    U1,
    U1U1,
    Z2,
    Z2Z2,
    ZN,
    Symmetry,
    get_symmetry,
    get_zn_symmetry_cls,
)

__all__ = (
    "AbelianArray",
    "AbelianArrayFlat",
    "abs",
    "align_axes",
    "all",
    "any",
    "BlockIndex",
    "BlockVector",
    "build_local_fermionic_array",
    "build_local_fermionic_dense",
    "build_local_fermionic_elements",
    "build_local_spin_array",
    "build_local_spin_dense",
    "clip",
    "conj",
    "default_tensordot_mode",
    "einsum",
    "expand_dims",
    "fermi_double_occupancy_local_array",
    "fermi_hubbard_local_array",
    "fermi_hubbard_spinless_local_array",
    "fermi_number_down_local_array",
    "fermi_number_operator_spinful_local_array",
    "fermi_number_operator_spinless_local_array",
    "fermi_number_up_local_array",
    "fermi_pairing_bond_local_array",
    "fermi_pairing_onsite_local_array",
    "fermi_spin_minus_local_array",
    "fermi_spin_operator_local_array",
    "fermi_spin_plus_local_array",
    "fermi_spin_z_local_array",
    "FermionicArray",
    "FermionicArrayFlat",
    "FermionicOperator",
    "FlatVector",
    "fuse",
    "get_default_tensordot_mode",
    "get_spinful_charge_indexmap",
    "get_spinhalf_charge_indexmap",
    "get_spinless_charge_indexmap",
    "get_symmetry",
    "get_zn_symmetry_cls",
    "ham_fermi_hubbard_from_edges",
    "ham_fermi_hubbard_spinless_from_edges",
    "ham_heisenberg_from_edges",
    "ham_tfim_from_edges",
    "heisenberg_local_array",
    "imag",
    "isfinite",
    "lddiv",
    "ldmul",
    "linalg",
    "log",
    "log10",
    "log2",
    "max",
    "min",
    "multiply_diagonal",
    "parse_edges_to_site_info",
    "MPS_abelian_rand",
    "MPS_fermionic_rand",
    "PEPS_abelian_rand",
    "PEPS_fermionic_rand",
    "PEPS3D_abelian_rand",
    "PEPS3D_fermionic_rand",
    "rddiv",
    "rdmul",
    "real",
    "reshape",
    "scipy",
    "set_default_tensordot_mode",
    "spin_operator_local_array",
    "sqrt",
    "squeeze",
    "sum",
    "Symmetry",
    "take",
    "tensordot",
    "tfim_local_array",
    "TN_abelian_from_edges_rand",
    "TN_fermionic_from_edges_rand",
    "trace",
    "transpose",
    "U1",
    "U1Array",
    "U1FermionicArray",
    "U1U1",
    "U1U1Array",
    "U1U1FermionicArray",
    "utils_test",
    "utils",
    "Z2",
    "Z2Array",
    "Z2ArrayFlat",
    "Z2FermionicArray",
    "Z2FermionicArrayFlat",
    "Z2Z2",
    "Z2Z2Array",
    "Z2Z2FermionicArray",
    "ZN",
)
